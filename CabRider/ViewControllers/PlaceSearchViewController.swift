...
class PlaceSearchViewController : BaseViewController, UISearchBarDelegate{
    ...
    func viewDidAppear(_ animated: Bool) {
        ...
        searchBar?.tg_width.equal(view.frame.width)
        searchBar?.tg_height.equal(60)
        searchBar?.barTintColor = Style.BackgroundColor
        searchBar?.dividerColor = Style.BackgroundColor
        searchBar?.borderColor = Style.BackgroundColor
        searchBar?.backgroundColor = Style.BackgroundColor
        searchBar?.tintColor = Style.BackgroundColor
        searchBar?.searchBarStyle = .minimal
        searchBar?.tg_top.equal(20);
        searchBar?.delegate = self;
        if(isSrcMode == true){
            linearLayout.addSubview(getToolbar(title: "SEARCH PICKUP LOCATION", isBackMenu: true))
            searchBar?.placeholder = "Select Pickup Location"
            ...
            searchBar?.placeholder = "Select Drop Location"
